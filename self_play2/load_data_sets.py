...
import go
from sgf_wrapper import replay_sgf, PositionWithContext
import utils
import random
...
def find_sgf_files(dataset_root):
...
def split_test_train(positions_w_context, desired_test_size):
    positions_w_context = list(positions_w_context)
    random.shuffle(positions_w_context)
    return positions_w_context[:desired_test_size], positions_w_context[desired_test_size:]
...
class DataSet(object):
    ...
    def write(self, filename):
        self.pos_features = self.pos_features.astype(np.uint8)
        self.next_moves = self.next_moves.astype(np.uint8)
...
def parse_data_sets(dataset_root, desired_test_size):
    sgf_files = list(find_sgf_files(dataset_root))

    print("%s sgfs found." % len(sgf_files), file=sys.stderr)
    positions_w_context = itertools.chain(*map(get_positions_from_sgf, sgf_files))
    desired_test_size = int(desired_test_size * len(positions_w_context))
    test_chunk, train_chunk = split_test_train(positions_w_context, desired_test_size)
    return test_chunk, train_chunk

def parse_selfplay_sets(dataset_root, processed_dir, desired_test_size):
    sgf_files = list(find_sgf_files(dataset_root))
    sgf_file = sgf_files[0]
    ds = DataSet.read(sgf_file)
    pos_features = np.empty([0,ds.board_size, ds.board_size, ds.input_planes])
    next_moves = np.empty([0,ds.board_size * ds.board_size])
    del sgf_files[0]
    for sgf_file in sgf_files:
        ds = DataSet.read(sgf_file)
        pos_features = np.concatenate((pos_features, ds.pos_features), axis=0)
        next_moves = np.concatenate((next_moves, ds.next_moves), axis = 0)
    assert len(pos_features) == len(next_moves)
    pos_features, next_moves = shuffle(pos_features, next_moves, random_state=0)
    desired_test_size = int(desired_test_size * len(pos_features))
    print("=====Test # %s, Train # %s.=====" % (desired_test_size, len(pos_features) - desired_test_size))
    test_dataset = DataSet(pos_features[:desired_test_size], next_moves[:desired_test_size], [])
    train_dataset = DataSet(pos_features[desired_test_size:], next_moves[desired_test_size:], [])
